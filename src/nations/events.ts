...
events.playerJoin.on(ev => {
    ...
    if (database.exist_file(root.DATABASE_PLAYER, `${xuid}.json`)) {
        ...
        nations_players.set(`${xuid}`, new TerritoryPlayer(false, player_name_xuid));
    }
});
//들어오고 나갈때 동화율 조정
/*
...
events.blockInteractedWith.on(ev => {
    ...
    const area_territory: TerritoryArea | undefined = nations_areas.get(chunk.get_dxz_chunk_line());
    const player_territory: TerritoryPlayer | undefined = nations_players.get(player.getXuid())!;
