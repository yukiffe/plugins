...
command.register("tpa", "티피 요청을 합니다.").overload(
    (param, origin, output) => {
        ...
        for (const player of param.target.newResults(origin, ServerPlayer)) {
            ...
            bedrockServer.executeCommand(
                `tellraw "${player.getName()}" {"rawtext":[{"text":"§l§d${actor}§e님이 당신에게 티피요청을 보냈습니다 30초 뒤에 취소 됩니다. §b\n수락 하는방법: /tpaccept"}]}`,
            );
...
command.register("tpaccept", "티피 요청을 받습니다.").overload((param, o, op) => {
    ...
    if (player?.isPlayer) {
        ...
        if (tpas[ni.getActor()!.getName()] !== "") {
            ...
            bedrockServer.executeCommand(
                `tellraw "${tpa[ni.getActor()!.getName()]}" {"rawtext":[{"text":"§l§e${player.getName()}§a님에게 정상적으로 티피 되었습니다."}]}`,
            );
            tpa[player.getName()] = "";
            fs.writeFileSync(`tpa.json`, JSON.stringify(tpa), "utf8");
        }
...
command.register("tpadeny", "티피 요청을 거절 합니다.").overload(
    (param, origin, op) => {
